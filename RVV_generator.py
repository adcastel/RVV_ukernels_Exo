...
def ukr_rvv(MR,NR,prec,LANE,beta0,swapAB=False, gather=False):
    @proc
    def ukernel_beta0(
            KC: size,
            MR: size,
    ...
    def make_tail(t,desp,gather=False):
        if gather:
        ...
        t = lift_alloc(t, 'C_regt', n_lifts=3 if gather == False else 4)
        t     = autofission(t, t.find('{}[_] = _'.format('C_regt')).after(), n_lifts=3 if gather == False else 4)
        if gather:
        ...
        t = lift_alloc(t, Xreg, n_lifts=3 if gather == False else 4)
        t = autofission(t, t.find('{}[_] = _'.format(Xreg)).after(),n_lifts=2 if gather == False else 3)
    ...
    def make_tail_ok(t, gather=False):
        ...
        t = lift_alloc(t, 'C_reg', n_lifts=3 if gather == False else 4)
        if gather and NR % LANE != 0:
            t = lift_alloc(t, 'C_reg2', n_lifts=3)

        t     = autofission(t, t.find('{}[_] = _'.format('C_reg')).after(), n_lifts=3 if gather == False else 4)
        ...
        if gather:
                t = autofission(t, t.find('{}[jtt + {} * jt, i] = _'.format('C',LANE)).before(), n_lifts=3 if gather == False else 4)
                if NR % LANE != 0:
        # 6) If we are in a tail case we need to move k loop and then fuse it with the normal case one
            ...
            t = autofission(t, t.find('{}[j,i] = _'.format('C')).before(), n_lifts=3 if gather == False else 4)
        t = simplify(t)
        ...
        # END IF GATHER FOR B
        if swapAB == True:
            jj = 'j' if gather == False else 'jt'
            if gather == True and NR % LANE != 0:
                pass
            else:
    ...
    p = lift_alloc(p, Xreg, n_lifts=4 if gather == False else 5)
    p = autofission(p, p.find('{}[_] = _'.format(Xreg)).after(),n_lifts=3 if gather == False else 4)
    ...
    if MR % LANE != 0:
        p = make_tail(p, (MR//LANE)*LANE, gather)
    ...
    if MR % LANE != 0:
        p = reorder_up(p, "C_regt : _",n=5 if gather == False else 6)
        ll = 'j' if gather == False else 'jt'
        p = reorder_up(p, f"for {ll} in _:_ #4",n=1)
